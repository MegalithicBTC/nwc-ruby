  ...
  %i[nip44_v2 nip04].each do |enc|
    context "with #{enc}" do
      it 'round-trips a successful make_invoice response' do
        ...
        req_event = NwcRuby::NIP47::Request.build(
          method: 'make_invoice',
          params: { 'amount' => 1_000 },
        ...
        inbound_plaintext =
          if enc == :nip44_v2
            NwcRuby::NIP44::Cipher.decrypt(req_event.content, wallet_priv, client_pub)
          else
            NwcRuby::NIP04::Cipher.decrypt(req_event.content, wallet_priv, client_pub)
          end
        ...
        response_ct =
          if enc == :nip44_v2
            NwcRuby::NIP44::Cipher.encrypt(response_payload.to_json, wallet_priv, client_pub)
          else
            NwcRuby::NIP04::Cipher.encrypt(response_payload.to_json, wallet_priv, client_pub)
          end

        response_event = NwcRuby::Event.new(
          pubkey: wallet_pub,
        ...
        parsed = NwcRuby::NIP47::Response.parse(response_event, client_priv, wallet_pub)
      ...
      it 'parses an error response' do
        ...
        response_event = NwcRuby::Event.new(
          pubkey: wallet_pub,
          kind: NwcRuby::NIP47::Methods::KIND_RESPONSE,
          tags: [['p', client_pub], %w[e fake-request-id]],
          content: response_ct
        ).sign!(wallet_priv)
